...
The performance of [Corels](https://corels.eecs.harvard.edu/) on test data is compared to [XGboost](https://xgboost.readthedocs.io/en/latest/R-package/xgboostPresentation.html). The precision XGBoost achieved on unseen test data is **84%** (106 / (106 + 20)) vs. **78%** (117/(117+34)) for Corels in this example. The null accuracy (always predicting the most frequent class of a good loan) is **73%** (142 / (142 + 52).

Precision is an important metric for loan decisions of precision (i.e. when a good loan is predicted, how often it is true). While XGBoost performs better, the Corels rules are short and easy to interpret. And the accuracy achieved by the two methods is more similar (XGBoost **72%** vs. Corels **70%**).
...
The [german credit data](https://www.openml.org/d/31) is downloaded and prepared using the code in [tidymodels with drake](https://github.com/hamedbh/test-drake-tidymodels). 
...
First let's quickly explore the [relationships](https://livebook.datascienceheroes.com/exploratory-data-analysis.html#selecting_best_vars_mic) between each predictor and the outcome of a good or bad loan.
...
We also store the cut points of the categorised continuous columns to be used later when using Corels.
...
In each plot below we can view the relationships between each predictor and outcome. Variables that have the most variation in the ratio between good and bad loans we would expect them to feature in the classification models. For example, acctStatus, durationCat, creditHistory, purpose, and amountCat. 
...
Using the recipes package in [tidymodels](https://www.tidymodels.org/) we create a recipe that converts the three continuous value columns into categories, then converted the values in every column into individual dummy fields per value.
...
## Iterative Bayesian optimisation
...
Compared to Corels, the most important metric is precision. This is because incorrectly labelling bad loans as good has the greatest cost.
