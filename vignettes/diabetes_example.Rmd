...
```{r, out.width = "80%"}
diabetes_train_model$alluvial_df %>%
...
The accuracy achieved by Corels on the unseen test data is `r conf_matrix$.estimate[1]`. This is the same accuracy the random forest reached in Rebbeca's example. In contrast, Corels rules are easy to read and visualise with an alluvial plot. For example, below if we follow one green path through the Corels rules where diabetes was correctly labelled. We can see they are:

1. in older age groups (age_bin1 = 0) 
2. have the highest blood glucose concentraion (glucose_bin4 = 1), 
3. have insulin that is one of the three highest bins (insulin_bin1 = 0), 
4. their pedigree (family history) is in one of the three highest value bins, (pedigree_bin1 = 0) and 
5. they are in the highest triceps bin (skin fold thickness) (triceps_bin4 = 0).  
